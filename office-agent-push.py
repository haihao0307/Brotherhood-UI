#!/usr/bin/env python3
"""
Brotherhood-UI - Agent 状态主动推送脚本

用法：
1. 填入下面的 JOIN_KEY（你拿到的一次性 join key）
2. 填入 AGENT_NAME（你想要在看板里显示的名字）
3. 运行：python office-agent-push.py
4. 脚本会自动先 join（首次运行），然后每 15s 向 Brotherhood-UI 推送一次你的当前状态
"""
...
AGENT_NAME = "" # 必填：你在看板里的名字
OFFICE_URL = "https://office.example.com"  # 看板地址（按实际部署修改）
...
def do_join(local):
    ...
    if r.status_code in (200, 201):
        ...
        if data.get("ok"):
            ...
            print(f"✅ 已加入 Brotherhood-UI，agentId={local['agentId']}")
            return True
    print(f"❌ 加入失败：{r.text}")
    return False
